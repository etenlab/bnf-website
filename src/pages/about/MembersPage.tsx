function MembersPage() {
    return (
        <>
            <h1 id='committed-to-open-source-and-the-bnf'>
                Committed to Open Source and the BNF
                <a
                    className='headerlink'
                    href='#committed-to-open-source-and-the-bnf'
                    title='Permalink'
                >
                    ¶
                </a>
            </h1>
            <p>
                BNF Members are those who have proven their commitment to the BNF by contributing to
                one or more of the Foundation’s projects. We thank them for their ongoing
                contributions to the Foundation.
            </p>
            <p>
                Our Members are similar to stockholders–they elect new Members and may vote for
                Directors. Membership in The BiblioNexus Foundation is a privilege and is by
                invitation only. The <a href='/foundation/governance/'>Governance</a> page provides
                details on Membership and the nomination process.
            </p>

            <h2 id='bnf-members'>
                BNF Members
                <a className='headerlink' href='#bnf-members' title='Permalink'>
                    ¶
                </a>
            </h2>
            <p>
                This table shows current and emeritus members. If you are a member and want to be
                included (this is optional), please email development at biblionexus dot org.
            </p>
            <table className='table'>
                <thead>
                    <tr>
                        <th>Id</th>
                        <th>Name</th>
                        <th>Projects</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>michaelmarshall</td>
                        <td>Michael Marshall</td>
                        <td>
                            <a href='https://crowd.rocks'>crowdrocks</a>
                        </td>
                    </tr>
                </tbody>
            </table>

            <p></p>
        </>
    );
}

export default MembersPage;
